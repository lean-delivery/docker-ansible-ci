...
RUN apk add -U python3 bash jq git && \
    apk add --no-cache --virtual .build-deps python3-dev gcc rsync make openssl-dev libffi-dev musl-dev linux-headers openssh-client \
    ...
    pip3 install --no-cache azure-cli==2.0.43 && ln -s /usr/bin/python3 /usr/bin/python
