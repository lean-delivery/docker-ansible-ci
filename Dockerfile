...
RUN apk add --no-cache gcc rsync make openssl-dev libffi-dev musl-dev linux-headers openssh-client docker \
    && rm -rf /var/cache/apk/*
RUN pip3 install --no-cache --upgrade pip wheel pywinrm[credssp] boto boto3 \
    ansible[azure]==$ANSIBLE_VERSION pyOpenSSL PyYAML pytest molecule==$MOLECULE_VERSION \
