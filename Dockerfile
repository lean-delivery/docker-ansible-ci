...
#   molecule==2.21.dev46
RUN /usr/local/bin/python -m pip install molecule==$MOLECULE_VERSION
RUN /usr/local/bin/python -m pip install --upgrade ansible==$ANSIBLE_VERSION docker pyOpenSSL PyYAML pytest
RUN /usr/local/bin/python -m pip install pywinrm[credssp] boto boto3
RUN /usr/local/bin/python -m pip install git+https://github.com/ansible/ansible-lint.git && \
    ansible --version && \
